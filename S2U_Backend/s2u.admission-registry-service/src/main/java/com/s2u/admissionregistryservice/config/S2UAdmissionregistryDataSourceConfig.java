 ...
 * "com.s2u.admissionregistryservice.repository", entityManagerFactoryRef =
 * "s2uEntityManager", transactionManagerRef = "s2uTransactionManager") public
 * class S2UAdmissionregistryDataSourceConfig {
 * 
 * private static final Logger log =
 * LoggerFactory.getLogger(S2UAdmissionregistryDataSourceConfig.class);
 * 
 * @Autowired Environment env;
 * 
 * @Primary
 * 
 * @Bean public LocalContainerEntityManagerFactoryBean s2uEntityManager() {
 * final LocalContainerEntityManagerFactoryBean em = new
 * LocalContainerEntityManagerFactoryBean();
 * em.setDataSource(s2uAdmissionregistryDataSourceConfig());
 * em.setPackagesToScan("com.s2u.admissionregistryservice.entity"); final
 * HibernateJpaVendorAdapter vendorAdapter = new HibernateJpaVendorAdapter();
 * em.setJpaVendorAdapter(vendorAdapter); final HashMap<String, Object>
 * properties = new HashMap<>(); properties.put("hibernate.hbm2ddl.auto",
 * env.getProperty("hibernate.ddl-auto")); properties.put("hibernate.dialect",
 * env.getProperty("spring.jpa.database-platform"));
 * em.setJpaPropertyMap(properties);
 * log.debug("Custom datasource initialized {}", em.getJpaPropertyMap()); return
 * em; }
 * 
 * @Primary
 * 
 ...
 * "s2u.admission-registry-service.datasource") public DataSource
 * s2uAdmissionregistryDataSourceConfig() { return
 * DataSourceBuilder.create().build(); // return dataSource; } }
 * 
 * @Primary
 * 
 * @Bean public PlatformTransactionManager s2uTransactionManager() { final
